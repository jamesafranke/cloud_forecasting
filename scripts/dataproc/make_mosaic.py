...
def blend(x1, x2):
    delta = np.median( x1[:,-1] ) - np.median(x2[:,0] ) 
    delta = np.linspace( delta, 0, np.floor(x2.shape[1]/2).astype(int) )
    delta = np.pad( delta, (0, np.ceil(x2.shape[1]/2).astype(int)) )
    return np.hstack( (x1, x2 + delta) )

datelist = pd.date_range('2019-01-01T00:00:00', '2019-01-03T00:00:00', freq='30min').tolist()

for t in datelist:
    if len( glob(f'{root}processed/{t.year}{t.month:02}{t.day:02}{t.hour:02}{t.minute:02}_B*.npy') ) < 11:
        ...
        for i in [10]: #range(1):
            path = f'{root}processed/{t.year}{t.month:02}{t.day:02}{t.hour:02}{t.minute:02}_B{i:02}.npy'
